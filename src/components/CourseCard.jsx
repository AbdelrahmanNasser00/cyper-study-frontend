...
const CourseCard = ({
  ...
  // Use either originalPrice or discountPrice based on availability
  ...
  const discount = finalOriginalPrice ? Math.round(((finalOriginalPrice - finalPrice) / finalOriginalPrice) * 100) : 0;
      ...
      <div className="flex">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            ...
            size={14}
            className={`${
              star <= Math.round(rating) ? 'fill-[#f9b15e] text-[#f9b15e]' : 'text-gray-300'
            }`}
          />
        ))}
      </div>
      <span className="text-xs text-muted-foreground">({finalReviewsCount.toLocaleString()})</span>
    </div>
